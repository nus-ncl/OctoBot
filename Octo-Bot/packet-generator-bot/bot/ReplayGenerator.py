...
def generateReplay(args):
    '''
    Generates (modified) replay traffic for every capture.
    ...
    for pcapFile in args.pcap:
        doReplay(pcapFile, args)

def doReplay(pcapFile, args):
    '''
    Generates (modified) replay traffic. (helper function)

    Parameters:
        pcapFile (File): capture file to replay
        args (Name space): program arguments
