...
def doUpload(params):
	...
	print(filename)
	print("Saving to:")
	# print(type(params['server']))
	# print(type(session.pwd()))
	# print(type(filename.split('/')[-1]))
	# print(type(suffix))

	# print(params['server'] + ':' + session.pwd() + '/' + filename.split('/')[-1]) + '_' + suffix
...
def doDownload(params):
	...
	print("Saving to:")
	print(os.path.dirname(os.path.abspath(__file__)) + '/' + filename.split('/')[-1])
	# download
